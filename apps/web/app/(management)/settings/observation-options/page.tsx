...
export default function ObservationOptionsPage() {
  ...
  const moveOption = async (option: Option, direction: -1 | 1) => {
    const siblings = options
      .filter(
        (item) => item.category_id === option.category_id && item.editable,
      )
  ...
  return (
    <main aria-labelledby="observation-options-title">
      ...
      <p>
        平台預設提供穩定
        Code；收容所擴充可以改名、排序與停用，歷史回報仍保留原始顯示快照。
      </p>
      <p role="alert" hidden={!errorMessage}>
        授權或操作失敗：{errorMessage}
      </p>
        ...
        <ul>
          {categories.map((category) => (
            <li key={category.id}>
              ...
              {category.source === "platform_default"
                ? "平台預設"
                : "收容所擴充"}
              ）
            </li>
        ...
        <form onSubmit={(event) => void createOption(event)}>
          ...
          <label>
            <input
              ...
              onChange={(event) => setNewRequiresNote(event.target.checked)}
            />
            需要補充說明
          </label>
      ...
      <section aria-labelledby="observation-list-title">
        <h2 id="observation-list-title">
          有效與歷史選項（可改名、排序或停用收容所擴充）
        </h2>
        <ul>
          {options.map((option) => (
            <li key={option.id}>
              <span>
                ...
                {categoryNameById.get(option.category_id) ?? "未知類別"}／
                {option.source === "platform_default"
                  ? "平台預設"
                  : "收容所擴充"}
                ...
                <>
                  ...
                  <button
                    ...
                    onClick={(event) => {
                      ...
                      void updateOption(option, {
                        display_name: input?.value ?? option.display_name,
                      });
                    }}
                  >
                    儲存改名
                  ...
                  {option.enabled && (
                    <button
                      type="button"
                      onClick={() =>
                        void updateOption(option, { enabled: false })
                      }
                    >
                      停用
                    </button>
