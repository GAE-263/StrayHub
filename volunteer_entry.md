...
| Task 9～12：Volunteer route/session lifecycle | 進行中（T005、T006完成） | T005 typed `AuthenticatedRouteContext`／`SessionSource`、LIFF transient entry reference與bounded recovery epoch storage、auth terminal source＋LIFF transient cleanup；entry reference強制32～512 URL-safe格式，recovery original path為strict volunteer allowlist/ID pattern；T006純函式`EffectiveRole`／route area／context-required／redirect／recovery finite-state decision matrix；authenticated VOLUNTEER所有management deep link固定導向`/animal-confirmation` | T005 focused `10 passed`、T006 focused `20 passed`、typecheck、T005 TypeScript scoped Prettier通過；主ledger markdown與兩個既有frontend format drift不納入；AuthenticatedRouteBoundary、children mount gate、animals handoff、401 recovery、shelter label尚未實作 | — |
