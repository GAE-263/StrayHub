...
| Task 9～12：Volunteer route/session lifecycle | 進行中（T006完成） | T006純函式`EffectiveRole`／route area／context-required／redirect／recovery finite-state decision matrix；authenticated VOLUNTEER所有management deep link固定導向`/animal-confirmation` | focused `20 passed`、typecheck、Prettier通過；AuthenticatedRouteBoundary、children mount gate、animals handoff、401 recovery、shelter label尚未實作 | — |
