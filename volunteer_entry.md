...
| Task 9～12：Volunteer route/session lifecycle | 進行中（T005、T006、T009完成） | T005 typed `AuthenticatedRouteContext`／`SessionSource`、LIFF transient entry reference與bounded recovery epoch storage、auth terminal source＋LIFF transient cleanup；entry reference強制32～512 URL-safe格式，recovery original path為strict volunteer allowlist/ID pattern；T006純函式`EffectiveRole`／route area／context-required／redirect／recovery finite-state decision matrix；T009繁中可及`ProtectedRouteState`，只有`allowed` render children，其他state不mount protected children，live region與interactive actions分離、shared 44px Button、context/access recovery actions完整；authenticated VOLUNTEER所有management deep link固定導向`/animal-confirmation` | T005 focused `10 passed`、T006 focused `20 passed`、T009 focused `10 passed`、typecheck、各切片TypeScript scoped Prettier通過；主ledger markdown與兩個既有frontend format drift不納入；AuthenticatedRouteBoundary composition、animals handoff、401 recovery、shelter label尚未實作 | — |
