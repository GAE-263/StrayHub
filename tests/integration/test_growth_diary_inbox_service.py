    ...
    try:
        ...
        async def scenario() -> None:
            async with session_factory() as session:
                ...
                updated = await service.set_status(concern_entry.id, entry_status="reviewed")
        ...
        async def scenario() -> None:
            async with session_factory() as session:
                ...
                with pytest.raises(Exception, match="不支援的狀態"):
                    await service.set_status(entry.id, entry_status="archived")
                with pytest.raises(Exception, match="找不到"):
                    await service.set_status(uuid4(), entry_status="reviewed")
