"""Resolve the tenant context used by LINE webhook events.

LINE identifies a person, not a shelter.  This service is deliberately small and
keeps the policy in one place so webhook handlers cannot accidentally infer a
tenant from an untrusted postback value.
"""

from __future__ import annotations

from datetime import datetime, timedelta, timezone

from services.api.app.api.errors import DomainError
from services.api.app.persistence.models.identity import WebhookSession


class LineWebhookSessionService:
    def __init__(self, identity, authentication, *, session_ttl_seconds: int = 86400) -> None:
        self.identity = identity
        self.authentication = authentication
        self.session_ttl_seconds = session_ttl_seconds

    async def resolve(self, line_user_id: str) -> WebhookSession:
        binding = await self.identity.binding(line_user_id)
        if binding is None:
            raise DomainError("line_binding_required", "請先完成 LINE 身分綁定", 403)

        user = await self.authentication.get_user(binding.user_id)
        if user is None or user.status != "active":
            raise DomainError("line_binding_invalid", "LINE 身分綁定無效", 403)

        memberships = await self.authentication.memberships(user.id, active_only=True)
        sessions = await self.identity.sessions(user.id)
        if len(sessions) > 1:
            raise DomainError("shelter_context_required", "請在 LIFF 明確選擇收容所", 409)
        if sessions:
            session = sessions[0]
            membership = next(
                (item for item in memberships if item.organization_id == session.organization_id),
                None,
            )
            if membership is None:
                raise DomainError("shelter_context_required", "請在 LIFF 明確選擇收容所", 409)
        elif len(memberships) == 1:
            membership = memberships[0]
            session = None
        else:
            raise DomainError("shelter_context_required", "請在 LIFF 明確選擇收容所", 409)
        organization = await self.authentication.get_organization(membership.organization_id)
        if organization is None or organization.status != "active":
            raise DomainError("organization_disabled", "收容所目前停用", 403)

        if session is not None:
            return session

        now = datetime.now(timezone.utc)
        return await self.identity.add(
            WebhookSession(
                user_id=user.id,
                organization_id=organization.id,
                status="active",
                expires_at=now + timedelta(seconds=self.session_ttl_seconds),
            )
        )
