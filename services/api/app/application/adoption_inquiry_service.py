...
class AdoptionInquiryInboxService:
    ...
    async def list(
        ...
        *,
        ...
        if search:
            ...
            filters.append(
                or_(
                    AdoptionInquiry.animal_name_snapshot.ilike(pattern),
                    AdoptionInquiry.shelter_number_snapshot.ilike(pattern),
                    AdoptionInquiry.adopter_name.ilike(pattern),
                    AdoptionInquiry.phone_number.ilike(pattern),
                )
            )
    ...
    async def set_status(
        ...
        inquiry = await self.session.get(AdoptionInquiry, inquiry_id)
        if inquiry is None or inquiry.organization_id != self.organization_id:
