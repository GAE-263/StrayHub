...
> 狀態：前端框架已建、mock 可運作；下列 **[待實作]** 端點需後端補上。
...
## 授權
- LIFF 取得 LINE `id_token` → `POST /v1/line/bind` 綁定，回傳 session 與角色。
- 僅 `STAFF` / `SHELTER_ADMIN` / `PLATFORM_ADMIN` 可用（後端以 `require_staff_or_admin` 把關）。
...
```json
{
  ...
  "submittedBy": { "lineUserId": "U...", "displayName": "王小明", "role": "staff" },
  "animal": {
...
```
建議回應：`{ "success": true, "animalId": "<正式或暫時ID>" }`。
註：`tempAnimalId` 為前端暫時編號，後端可沿用或換發正式收容編號。

### 4. `POST /v1/management/animals/{animalId}/health-records` — **[待實作]**
...
- 目前 `management_animals` 只有 list / get / patch(status)；本合約的 3、4 為新增。

## 前端切換正式環境
`line-liff/staff-animal/js/config.js` 填 `LIFF_ID`、`API_BASE_URL`、`MOCK_MODE=false`；
`js/api.js` 的 `USE_MOCK_API=false`。其餘頁面不需改。
