...
1. **Given** 工作人員或收容所管理員已登入並選定收容所，**When** 從管理導覽進入毛孩日記，**Then** 系統依最新到最舊顯示該收容所的日記。
...
- 照片短效存取連結可能過期或儲存服務暫時失敗；頁面必須保留文字、時間及摘要並提供圖片替代狀態。
...
- **FR-002**: 頁面 MUST 僅供經後端驗證的收容所工作人員或收容所管理員存取；前端隱藏導覽不得作為唯一授權控制。
...
- **FR-010**: 照片 MUST 使用既有受控存取方式顯示；圖片失敗或連結過期時不得導致日記其他內容消失。
