...
- Q: 標準回報哪些項目必須回答，以及照護與散步完成狀態如何記錄？ → A: 標準回報必須完成動物確認、照護完成狀態、散步完成狀態、進食、飲水、活動、排尿、排便、護食或資源防衛、對人的互動、對其他動物的互動、情緒、散步反應及外觀／特殊狀態。每個結構化題目都必須有答案；「未觀察」、「無法判斷」及適用的「未進行散步」是有效答案，不等同於略過。照片與心得仍為選填；只有選擇「其他」、被設定為需補充或志工主動補充時才要求文字。

### Session 2026-08-07

- 標準回報的必填結構化答案與照護／散步完成狀態已正式定義，並納入 `answering_completion` 流程步驟。此決策不代表已決定資料表欄位或 API 實作方式。
...
志工確認動物後，主要透過 LINE Quick Reply 與 Postback Action，逐題完成標準回報。標準回報必須回答照護完成狀態、散步完成狀態、進食、飲水、活動、排尿、排便、護食或資源防衛、對人的互動、對其他動物的互動、情緒、散步反應及外觀／特殊狀態。每個結構化題目都必須有答案；「未觀察」、「無法判斷」及「未進行散步」是有效答案，不會被視為略過。志工可以用 LINE 相機、相簿或圖片訊息附加照片；心得為選填，只有選擇「其他」、需要詳細說明或主動補充時才要求文字或開啟 LIFF。Bot 流程無法完成時，志工可以使用 LIFF 輔助介面。即使 AI 或其他外部服務不可用，原始回報仍能正式保存並立即得到保存成功的確認。

標準回報的完成條件是：已確認動物、所有上述結構化題目都有答案、摘要已由志工確認，且送出時權限與動物關聯重新驗證成功。志工不得以「略過」跳過必填結構化題目；若尚未完成任一題，系統不得進入正式送出，並應帶回第一個未回答題目。照片與心得不屬於標準回報的必填項目。
...
1. **Given** 志工已確認可回報動物，**When** 依序回答照護完成狀態、散步完成狀態、進食、飲水、活動、排尿、排便、護食或資源防衛、對人的互動、對其他動物的互動、情緒、散步反應及外觀／特殊狀態，並確認摘要後送出，**Then** 系統保存一筆可辨識回報，並立即顯示原始回報保存成功；各題選擇「未觀察」、「無法判斷」或適用的「未進行散步」仍視為已回答。
...
5. **Given** 志工送出前發現動物選錯，**When** 返回重新選擇，**Then** 系統再次顯示確認卡；原結構化答案與心得可暫時保留但必須標示為待重新確認，原照片不得自動沿用至新動物，志工確認新動物並逐項確認保留內容後才可繼續送出。
...
18. 已綁定志工完成動物確認，並回答照護完成狀態、散步完成狀態、進食、飲水、活動、排尿、排便、護食或資源防衛、對人的互動、對其他動物的互動、情緒、散步反應及外觀／特殊狀態後，成功建立一筆完整回報；各題選擇「未觀察」、「無法判斷」或適用的「未進行散步」仍視為有效回答。
...
- 志工未回答任何一個標準回報必填題目時，系統不得進入正式送出；選擇有效的「未觀察」、「無法判斷」或「未進行散步」不屬於未回答。
- 照護完成狀態與散步完成狀態必須分開記錄；「未提供照護」、「部分完成」、「未進行散步」、「未觀察」與「無法判斷」不得互相轉換或被解讀為正常完成。
...
- **FR-027**：志工在送出前更換動物時，系統 MUST 再次要求動物確認；原結構化答案與心得 MAY 暫時保留，但 MUST 標示為待重新確認，志工逐項確認後才可送出；原照片 MUST NOT 自動沿用至新動物，且系統 MUST 明確告知哪些內容保留、哪些需要重填或重新附加。重新選擇不得改變 Draft 的 Organization，亦不得繞過新動物的 Reportable Scope 與權限驗證。
...
- 標準回報的必填結構化答案固定包含照護完成狀態、散步完成狀態、進食、飲水、活動、排尿、排便、護食或資源防衛、對人的互動、對其他動物的互動、情緒、散步反應及外觀／特殊狀態；「未觀察」、「無法判斷」及適用的「未進行散步」是有效答案，不等同於略過。
- 照護完成狀態與散步完成狀態分開記錄，分別使用 `care_completion.*` 與 `walk_completion.*` 穩定 Code；照片與心得為選填，選擇「其他」或需補充的選項時才要求文字。
...
- **SC-001**：在未接受本系統專門訓練的測試志工中，至少 80% 能獨立透過 LINE Bot 完成一筆標準照護回報，且不需工作人員代填或代操作；標準流程須完成動物確認、照護完成狀態、散步完成狀態、進食、飲水、活動、排尿、排便、護食或資源防衛、對人的互動、對其他動物的互動、情緒、散步反應及外觀／特殊狀態，並完成送出確認。每個結構化題目選擇「未觀察」、「無法判斷」或適用的「未進行散步」都算完成該題；照片與心得可略過。
- **SC-002**：至少 80% 的測試參與者能在 90 秒內完成一筆標準 LINE Bot 回報；標準回報定義為動物已確認，且已回答照護完成狀態、散步完成狀態、進食、飲水、活動、排尿、排便、護食或資源防衛、對人的互動、對其他動物的互動、情緒、散步反應及外觀／特殊狀態。計時從 Bot 顯示第一個可操作問題開始，到系統顯示正式 Care Report 保存成功為止，AI 處理時間不計入；照片與心得不計入必要完成項目。
...
- SC-001 與 SC-002 MUST 使用至少 10 名未接受本系統專門訓練、且未參與本功能設計或實作的志工測試者；同一批測試者使用相同的標準 LINE Bot 回報情境與虛構資料。SC-001 至少 8 人必須獨立完成；SC-002 至少 8 人必須在 90 秒內完成，才算分別通過對應成功條件。測試記錄必須逐項確認上述必填結構化題目是否有答案；選擇有效的「未觀察」、「無法判斷」或「未進行散步」不得記為漏答，照片與心得不得列為失敗原因。
- SC-006 與 SC-014 MUST 使用至少 10 名未參與本功能設計或實作的工作人員測試者；至少 9 人必須在最多三次主要操作內進入指定 Animal 歷程並正確區分四種狀態。主要操作定義為造成頁面、路由或主要內容面板切換的一次點擊或觸控；輸入搜尋文字、捲動與展開同頁細節不另計一次主要操作。
- SC-002 的 90 秒計時、SC-006 的兩秒摘要時間與自動化效能量測可以由測試工具輔助記錄，但自動化 Integration／E2E Test MUST NOT 取代上述真人完成率與辨識率驗收。
- 真人驗收 MUST 使用虛構資料與非正式環境，並保存去識別化證據，至少包括測試批次、測試者代碼、角色群組、裝置類型、開始與結束時間、是否完成、是否接受協助、操作次數、狀態辨識答案及失敗原因；不得在驗收證據中保存 LINE User ID、真實姓名或正式收容所敏感資料。
- 若樣本超過 10 人，完成率與辨識率以全部有效樣本計算；不得排除失敗樣本以提高通過率，也不得以少於 10 人的結果宣稱達成 SC-001、SC-002、SC-006 或 SC-014。
...
- 本規格未發現與 Constitution 的衝突；Constitution v2.1.0 的批准日期已依修訂 commit `e8aebb2` 追溯為 2026-08-06。
