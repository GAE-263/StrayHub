...
建立收容所時至少要能記錄收容所名稱、機構代碼、地址或服務區域、聯絡資訊、啟用狀態、
初始管理員與建立時間；其中哪些欄位必填列於 Clarification Items。

### 收容所管理員

每個收容所可以有一名或多名收容所管理員。收容所管理員只能管理自己所屬收容所，依權限
建立或停用工作人員帳號、管理志工、動物、收容編號、籠舍、區域、動物 QR Code、今日可
回報名單與機構內部設定，也可以查看所屬收容所的日常照護歷程。收容所管理員不得查看或
管理其他收容所的帳號、動物、照片、回報或內部資訊。
...
志工必須歸屬至一個收容所，或由 CRM 記錄其被授權服務的收容所範圍。志工只能查看授權
收容所內完成照護回報所需的最少動物資訊、掃描授權收容所 QR Code、搜尋授權收容所內的
收容編號及回報被授權的動物。若未來允許同一志工服務多個收容所，系統必須清楚顯示目前
操作中的收容所，且每次回報只能歸屬至一個收容所；本期是否採用此模式列於 Clarification Items。
...
過濾。QR Code 掃描後必須同時確認 QR Code 所屬收容所、使用者可存取的收容所及動物所屬
收容所；三者一致且動物仍可回報，才可進入回報流程。修改 QR Code 內容不得繞過此檢查。
...
1. **Given** 平台管理員提供合法的新收容所資料，**When** 建立收容所，**Then** 收容所具有獨立機構範圍、服務狀態與初始管理員關聯。
...
- 同一志工若被授權服務多個收容所，若本期允許，操作畫面必須清楚顯示目前收容所，且每筆回報只能歸屬一個收容所；本期未決行為見 Clarification Items。
...
- **FR-004**：工作人員或管理員 MUST 能設定志工當日可回報範圍；設定結果 MUST 只允許符合權限的動物建立正式回報。
...
- **FR-008**：QR Code MUST 只承載候選動物查詢所需的穩定資訊，不得承載醫療內容、照護紀錄、個資、內部關注資訊或可被視為長效授權祕密；掃描本身不得代表已通過身分或權限驗證。
...
- **FR-028**：志工只能在允許期限內修改自己建立的回報；不得修改他人回報。未經確認的修改、刪除、封存與志工自助更正政策 MUST 依 Clarification Items 決定。
...
- **FR-053**：平台管理員 MUST 能依權限新增、修改、啟用或停用收容所，建立或協助重設初始收容所管理員，並查看收容所帳號與服務狀態；跨機構管理操作 MUST 有明確授權與稽核紀錄。
...
- **FR-056**：志工 MUST 歸屬至一個收容所或被記錄明確授權的收容所範圍；志工只能查看完成回報所需的最少資訊與回報被授權的動物。
...
- QR Code 只用於查詢候選 Animal，不代表身分驗證、回報授權或長效祕密。
- 正式回報關聯 CRM 的 Animal，不關聯 QR Code 原始文字或當時顯示名稱。
- 志工可能不記得動物名稱，但可以使用照片、籠位、區域或 Shelter Number 辨識。
- 工作人員或收容所管理員能在管理入口設定所屬 Shelter 的 Daily Reportable Scope；實際設定粒度待確認。
...
- **SC-002**：至少 80% 的測試參與者能在 90 秒內完成一筆標準回報；標準回報定義為已確認動物、填寫必要結構化選項、不含額外 AI 等待。
...
以下只保留最影響範圍、資料正確性、權限與使用流程的三項待釐清事項；其餘未決內容先依 Assumptions 與本規格的保守行為驗收。

1. **[NEEDS CLARIFICATION: 收容所建立必要欄位與啟用條件]**：平台管理員建立收容所時，收容所名稱、機構代碼、地址或服務區域、聯絡資訊、啟用狀態、初始管理員與建立時間中，哪些欄位為必填？收容所建立後是否立即啟用，或需要額外審核？此決定影響平台管理員流程與收容所能否開始建立業務資料。
   - 暫定驗收：收容所至少要有可辨識名稱、機構代碼、服務狀態與初始管理員關聯；未完成必要資料或未啟用前，不得建立一般業務資料。

2. **[NEEDS CLARIFICATION: 志工跨收容所授權範圍]**：第一階段志工是否只能歸屬一個收容所，或允許同一志工服務多個收容所？若允許多收容所，如何顯示目前操作中的收容所與切換範圍？每次回報仍必須只歸屬一個收容所。此決定影響志工身分、今日可回報名單、QR Code 與回報歸屬。
   - 暫定驗收：未明確授權前，志工只能操作所屬收容所；QR Code 或搜尋結果不得自動增加可存取收容所。

3. **[NEEDS CLARIFICATION: QR Code、今日名單、回報修改與草稿政策]**：第一階段 QR Code 是否只包含 Shelter Number，還是使用系統深層連結？沒有 Shelter Number 的 Animal 使用何種穩定查詢資訊？今日可回報範圍由誰設定及採何種粒度？志工送出後是否可在 24 小時內自行更正？照片、排泄照片、心得、食量量表與護食記錄方式、草稿保存期限、跨裝置恢復、重複送出、刪除或封存政策為何？
   - 暫定驗收：QR Code 僅提供候選查詢，不提供授權；未明確授權前不得建立正式回報；送出前保留已填內容；送出後不得覆蓋原紀錄；所有未決觀察欄位以可觀察選項及「未觀察／無法判斷」表達，並保留更正軌跡。
