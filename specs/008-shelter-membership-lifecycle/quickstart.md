...
1. Open `http://127.0.0.1:3001/shelters` and sign in as the local shelter admin.
2. Confirm the page title is `權限管理`.
3. Confirm the upper `志工` section contains VOLUNTEER and the lower `工作人員` section contains SHELTER_ADMIN and STAFF; the section title does not repeat role names.
4. Confirm SHELTER_ADMIN appears before STAFF, and each section is ordered `啟用中` → `已停用` → `已過期` → `授權已撤銷`.
5. Confirm archived memberships are absent from both normal sections.
6. Confirm no timezone controls or timezone explanation appears on the permission management page.
7. Confirm `建立帳號` and `查看已封存成員` are available without scrolling to the bottom.
...
4. Open `http://127.0.0.1:3001/shelters/archived`.
...
## Scenario 3: Re-enable disabled accounts and show revoked authorization

1. Find a disabled SHELTER_ADMIN or STAFF and select `重新啟用`.
2. Confirm the Membership becomes `啟用中`, the card returns to the active visual treatment, and an Audit record is written.
3. Find a VOLUNTEER whose latest authorization is revoked or expired.
4. Confirm the volunteer card remains muted, displays `授權已撤銷` or `授權已到期`, and does not offer a path that directly changes it to active.

## Scenario 4: Account creation modal
...
4. Create a STAFF account and confirm the modal closes, the form resets, and the new account appears under `工作人員`.
5. Try a duplicate username and confirm the modal remains open with a safe error message and no password leakage.

## Scenario 5: Responsive and tenant boundaries
...
PLAYWRIGHT_SKIP_WEBSERVER=1 PLAYWRIGHT_BASE_URL=http://127.0.0.1:3001 npm --prefix apps/web run test:e2e -- e2e/organization-management.spec.ts e2e/p1-management.spec.ts
```
