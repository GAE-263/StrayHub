...
實作會沿用現有觀察類別／選項、FastAPI 端點、PostgreSQL 租戶隔離與 AuditRecord，擴充回傳摘要、來源／狀態／最後修改資訊、封存生命週期與管理權限邊界。歷史使用判斷以可重建的觀察選項使用索引輔助，正式照護回報與 `answer_snapshots` 仍是歷史資料來源；任何選項異動都不覆寫既有回報快照。

契約以既有觀察詞彙 HTTP 路徑為相容基礎，新增明確的生命週期動作與管理回應資料；前端使用單次載入後的組合篩選與類別分組，符合目前約 13 個類別、約 500 個選項的規模目標。完整決策與替代方案記錄於 [research.md](research.md)。
...
**儲存**：現有 `observation_categories`、`observation_options`、`care_reports.answer_snapshots` 與 `audit_records`；新增一個可由照護回報重建的觀察選項歷史使用索引，並以 Alembic migration 增加 `archived` 狀態所需的資料約束與索引。正式 CRM 資料與歷史快照不移轉到獨立來源。
...
**效能目標**：在約 13 個類別與 500 個選項的代表性資料量下，使用者在一般網路條件下 2 秒內看到摘要或明確載入中狀態；搜尋與組合篩選在 2 秒內顯示結果。前端搜尋不得因展開全部類別而造成長清單初始閱讀負擔。

**限制**：平台預設選項在收容所管理範圍唯讀；收容所自訂 stable code 在目前收容所可見的完整有效詞彙範圍內唯一；沒有硬刪除；已使用 code 鎖定；停用／封存不影響歷史回報；Staff 只能唯讀；所有異動要有 AuditRecord；所有查詢與寫入受目前 Shelter Context 與 PostgreSQL RLS 約束；不得要求 LINE、AI 或 GCP 服務才能完成本功能驗收。
...
- **VIII. 權限、隱私與稽核預設啟用：通過。** Staff 讀取、SHELTER_ADMIN／具設定管理權限者寫入；所有操作沿用 AuditService 並限制於已驗證 Shelter Scope。
...
- **XI. 多收容所資料隔離：通過。** 使用索引帶 `organization_id` 並受 RLS；管理者不得以 option id、搜尋、網址或 context 外資料讀寫其他收容所。

**Gate 結論**：通過，沒有需要以例外方式降低 constitution 要求的設計。

### Gate：Phase 1 後

**重新檢查結果：通過。** `data-model.md` 將使用索引定義為可由 CareReport／snapshot 重建的衍生資料，migration 要求不覆寫既有快照；`ui-behavior.md` 與 HTTP contract 分離 Staff 唯讀及 SHELTER_ADMIN／PLATFORM_ADMIN 管理能力；HTTP contract 保留既有 `source` 與 `enabled` 欄位並新增 `archived` 與明確狀態動作；每個新增、更新、排序、停用、恢復與封存都要求對應 AuditRecord。Phase 1 沒有新增 constitution 例外。
...
**Structure Decision**：採現有 Web application monorepo 的前端 feature module、FastAPI API／Application／Repository／Model 分層與 Alembic migration。頁面路徑保留，將現有單一頁面拆成可測試的觀察詞彙 feature 元件；API 保留現有五個讀寫路徑，另外為狀態轉換提供明確的動作路徑；契約 source 仍由既有 project contract 流程統一生成 `packages/contracts/src/openapi.ts`。

## 實作順序與交付邊界

1. **Foundational data and safety**：補齊 `archived` 狀態、使用索引、歷史快照補充欄位、租戶隔離、stable code 格式／唯一性、admin-only mutation 與 Audit payload。
2. **API contract**：擴充摘要、類別計數、來源／狀態／最後修改資訊與 `has_historical_usage`；加入 disable／restore／archive 明確動作；保留既有 response 欄位與無 DELETE 保證。
3. **Frontend information architecture**：建立摘要、來源說明、搜尋／篩選、收合類別、自訂優先、唯讀平台預設、表單、確認對話與狀態元件。
